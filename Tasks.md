...
- [x] T-008 .txt / 青空文庫テキストパーサ — テキスト読込、文字コード判定、青空文庫前処理を実装
- [x] T-009 EPUBパーサ（jszip） — EPUB 展開、OPF 解析、章抽出、ルビ変換を実装
- [x] T-010 パーサーWorker — txt / aozora / epub 解析を Worker 経由で実行
- [x] T-011 文節分割Worker — 大型書籍の文節分割と RubyToken 対応を Worker 化
- [x] T-012 インポートフロー（ライブラリページ） — ファイル取込、重複防止、DB 保存、一覧表示を実装
- [x] T-013 サンプル書籍同梱 — 青空文庫由来サンプルと初回投入処理を追加
- [x] T-014 リーダーストアと再生エンジン — 再生状態、タイマー、進捗保存を実装
- [x] T-015 ChunkStage（中央表示+フェード） — 文節中央表示と RubyText、クロスフェードを実装
- [x] T-016 リーダーページ（コントロール統合） — Reader UI、タップ/スワイプ操作、設定反映を統合
...
- [x] T-025 主要ロジックのユニットテスト網羅 — segmenter、ruby、parser、DB、reader のテストを拡充
...
- [x] T-032 トップページデザイン刷新（行燈ランディング） — 行燈（Andon）バリアントをTSX移植し、globals.cssにテーマトークン追加・useBunsetsuCycleフック新規作成・ライブ文節デモをヒーローに配置
...
- [x] T-038 しおり機能の追加 — Dexieに bookmarks テーブル追加、ヘッダー☆で現在位置の保存/解除、ライブラリページにしおり一覧導線、リーダー復帰時にしおり位置にジャンプ可能化
- [x] T-039 リーダーUIミニマル化 — 底部ControlBar削除・右SideRails削除・ProgressBarをより細く・右下に表示非表示可能な操作ヘルプ（?ボタン）を追加
- [x] T-040 デフォルト表示速度の調整 — DEFAULT_SETTINGS.displayDuration を 0.8秒 → 2.7秒/文節にゆったり化、トップページのライブデモも 1100ms → 2600ms に同期
